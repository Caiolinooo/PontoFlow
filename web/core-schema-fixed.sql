-- Schema v1.1 corrigido — Timesheet Manager (multi-tenant) — PostgreSQL compatible
-- Removidos 'if not exists' incompatíveis com PostgreSQL nas policies
...
-- Políticas sem 'if not exists' (PostgreSQL compatível)
...
-- Gerente: pode atualizar timesheets de colaboradores dos grupos delegados
...
-- Anotações
create policy timesheet_annotations_select on public.timesheet_annotations
  for select using (
    exists (
...
create policy timesheet_entries_view on public.timesheet_entries
  for select using (
    exists (
      select 1 from public.timesheets t
      join public.employees e on e.id = t.employee_id
...
-- Entries: funcionário modifica antes do prazo; gerente sempre
...
create policy timesheet_entries_manager_modify on public.timesheet_entries
  for all using (
    exists (
      select 1 from public.timesheets t
      join public.employees e on e.id = t.employee_id
