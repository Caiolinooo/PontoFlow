...
export default function GroupDetailPanel({
  ...
  const addManager = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('add_manager_failed');
  ...
  const removeManager = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('remove_manager_failed');
  ...
  const addMember = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('add_member_failed');
          ...
          <input
            ...
            onChange={(e) => {
              ...
              mgrSearchDebounce.current = window.setTimeout(async () => {
                if (!q) { setMgrResults([]); return; }
                const res = await fetch(`/api/admin/search/managers?q=${encodeURIComponent(q)}`);
                const j = await res.json().catch(() => ({}));
                setMgrResults((j.items || []));
              }, 300);
            }}
          />
