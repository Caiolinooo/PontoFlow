      ...
      {open && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40">
          <div className="bg-white rounded shadow-lg w-full max-w-lg p-4">
            <h3 className="font-medium mb-2">{t('rejectTitle')}</h3>
            <div className="space-y-3">
              <div>
                <label className="block text-sm mb-1">{t('rejectReason')}</label>
              ...
              <div className="flex items-center justify-between">
                <div className="font-medium text-sm">{t('addAnnotation')}</div>
              ...
              {annotations.map((a, idx) => (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 items-end" key={idx}>
                  <div>
                    <label className="block text-xs mb-1">{t('annotationEntry')}</label>
                  ...
                  <div>
                    <label className="block text-xs mb-1">{t('annotationField')}</label>
                  ...
                  <div className="md:col-span-1">
                    <label className="block text-xs mb-1">{t('annotationMessage')}</label>
                    <div className="flex gap-2">
            ...
            <div className="mt-4 flex gap-2 justify-end">
              <button className="px-3 py-2 border rounded text-sm" onClick={() => setOpen(false)}>{tActions('cancel')}</button>
              <button disabled={loading || !reason.trim()} className="px-3 py-2 rounded bg-red-600 text-white text-sm" onClick={onReject}>{tActions('submit')}</button>
            </div>
          </div>
        </div>
      )}
